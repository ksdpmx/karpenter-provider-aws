...
func BuildDisruptionBudgetMapping(
	...
	for _, nodePool := range nodePools {
		...
		fmt.Printf(
			"DEBUG nodePool %s, allowedDisruptions %d, disrupting %d, numNodes %d, reason: %s, disruptionBudgetMapping: %d\n",
			nodePool.Name,
			allowedDisruptions,
			disrupting[nodePool.Name], numNodes[nodePool.Name], reason,
			disruptionBudgetMapping[nodePool.Name],
		)
