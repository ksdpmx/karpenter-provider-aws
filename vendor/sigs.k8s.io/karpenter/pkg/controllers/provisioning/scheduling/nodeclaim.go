...
func (n *NodeClaim) CanAdd(
	ctx context.Context, pod *corev1.Pod, podData *PodData,
) (
	updatedRequirements scheduling.Requirements, updatedInstanceTypes []*cloudprovider.InstanceType,
	offeringsToReserve []*cloudprovider.Offering, err error,
) {
	// Check Taints
	...
	if err := nodeClaimRequirements.Compatible(
		podData.Requirements, scheduling.AllowUndefinedWellKnownLabels,
	); err != nil {
	...
	topologyRequirements, err := n.topology.AddRequirements(
		pod, n.NodeClaimTemplate.Spec.Taints, podData.StrictRequirements, nodeClaimRequirements,
	...
	if err = nodeClaimRequirements.Compatible(
		topologyRequirements, scheduling.AllowUndefinedWellKnownLabels,
	); err != nil {
		return nil, nil, nil, err
	}
...
func (n *NodeClaim) Add(
	pod *corev1.Pod, podData *PodData, nodeClaimRequirements scheduling.Requirements,
	instanceTypes []*cloudprovider.InstanceType, offeringsToReserve []*cloudprovider.Offering,
) {
	// Update node
...
func (n *NodeClaim) FinalizeScheduling() {
	...
	if len(n.reservedOfferings) != 0 {
		...
		n.Requirements[v1.CapacityTypeLabelKey] = scheduling.NewRequirement(
			v1.CapacityTypeLabelKey, corev1.NodeSelectorOpIn, v1.CapacityTypeReserved,
		)
		n.Requirements.Add(
			scheduling.NewRequirement(
				...
				lo.Map(
					n.reservedOfferings, func(o *cloudprovider.Offering, _ int) string { return o.ReservationID() },
				)...,
			),
		)
	}
...
func (n *NodeClaim) RemoveInstanceTypeOptionsByPriceAndMinValues(
	reqs scheduling.Requirements, maxPrice float64,
) (*NodeClaim, error) {
	n.InstanceTypeOptions = lo.Filter(
		n.InstanceTypeOptions, func(it *cloudprovider.InstanceType, _ int) bool {
...
func (e InstanceTypeFilterError) Error() string {
	...
	if e.minValuesIncompatibleErr != nil {
		return fmt.Sprintf(
			"%s, requirements=%s, resources=%s", e.minValuesIncompatibleErr.Error(), e.requirements,
			resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	}
	...
	if !e.requirementsMet && !e.fits && !e.hasOffering {
		return fmt.Sprintf(
			"no instance type met the scheduling requirements or had enough resources or had a required offering, requirements=%s, resources=%s",
			e.requirements, resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	}
	// check the other pairwise criteria
	if !e.requirementsMet && !e.fits {
		return fmt.Sprintf(
			"no instance type met the scheduling requirements or had enough resources, requirements=%s, resources=%s",
			e.requirements, resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	}
	if !e.requirementsMet && !e.hasOffering {
		return fmt.Sprintf(
			"no instance type met the scheduling requirements or had a required offering, requirements=%s, resources=%s",
			e.requirements, resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	}
	if !e.fits && !e.hasOffering {
		return fmt.Sprintf(
			"no instance type had enough resources or had a required offering, requirements=%s, resources=%s",
			e.requirements, resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	}
	...
	if !e.requirementsMet {
		return fmt.Sprintf(
			"no instance type met all requirements, requirements=%s, resources=%s", e.requirements,
			resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	}
	if !e.fits {
		msg := fmt.Sprintf(
			"no instance type has enough resources, requirements=%s, resources=%s", e.requirements,
			resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	...
	if !e.hasOffering {
		return fmt.Sprintf(
			"no instance type has the required offering, requirements=%s, resources=%s", e.requirements,
			resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	}
	...
	if e.requirementsAndFits {
		return fmt.Sprintf(
			"no instance type which met the scheduling requirements and had enough resources, had a required offering, requirements=%s, resources=%s",
			e.requirements, resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	}
	if e.fitsAndOffering {
		return fmt.Sprintf(
			"no instance type which had enough resources and the required offering met the scheduling requirements, requirements=%s, resources=%s",
			e.requirements, resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	}
	if e.requirementsAndOffering {
		return fmt.Sprintf(
			"no instance type which met the scheduling requirements and the required offering had the required resources, requirements=%s, resources=%s",
			e.requirements, resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
		)
	}
	...
	return fmt.Sprintf(
		"no instance type met the requirements/resources/offering tuple, requirements=%s, resources=%s", e.requirements,
		resources.String(resources.Merge(e.daemonRequests, e.podRequests)),
	)
}
...
func filterInstanceTypesByRequirements(
	instanceTypes []*cloudprovider.InstanceType, requirements scheduling.Requirements,
	podRequests, daemonRequests, totalRequests corev1.ResourceList,
) (cloudprovider.InstanceTypes, error) {
